...
static char *alg_name[] = { "dfc", "dfc2.c" };
...
void mult_64(u4byte r[4], const u4byte x[2], const u4byte y[2])
...
void add_64(u4byte r[4], const u4byte hi, const u4byte lo)
{
    if((r[0] += lo) < lo)
        if(!++r[1])
            if(!++r[2])
